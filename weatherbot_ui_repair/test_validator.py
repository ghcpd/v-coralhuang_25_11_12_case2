...
with sync_playwright() as p:
    ...
    duplicate_ids = page.evaluate("() => { const ids = Array.from(document.querySelectorAll('[id]')).map(e=>e.id); return ids.filter((v,i,a)=>a.indexOf(v)!==i) }")

    REPORT['accessibility_smoke'] = REPORT['accessibility_smoke'] and (not missing_role_rendered) and (not has_alt_missing) and (len(duplicate_ids)==0)
